...
class RemoteExecutionBackend(ExecutionBackend):
    def __init__(self, host: str, remote_host: RemoteHost):
        self.host = host
    ...
    async def exec(self, command: str, working_dir: str | None = None, timeout: float = 30.0) -> dict[str, Any]:
        ...
        result = await self.remote_host.exec(full_command, timeout=timeout)
    ...
    async def read_file(self, path: str) -> dict[str, Any]:
        return await self.remote_host.read_file(path, timeout=30.0)
    ...
    async def list_dir(self, path: str) -> dict[str, Any]:
        return await self.remote_host.list_dir(path, timeout=30.0)
